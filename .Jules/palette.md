...
## 2024-05-18 - Astro Header and Global Link Hover States
**Learning:** Default Astro blog templates often lack explicit visual feedback for link hover states (e.g., `a:hover` colors identically match `a`). Furthermore, when modifying variables like `--black` that are defined as comma-separated RGB values, they must be wrapped in `rgb()` (e.g., `rgb(var(--black))`) rather than used directly, or they generate invalid CSS.
**Action:** Always explicitly verify and apply distinct hover styles and transitions to interactive elements like global links and header navigation to improve UX. Double-check how color variables are formatted in the root CSS variables.
