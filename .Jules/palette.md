...
## 2023-10-27 - Missing link hover states and invalid CSS variable usage in Astro default template
**Learning:** The default Astro template often lacks explicit visual feedback for link hover states (e.g., `a:hover` colors identically match `a`). Additionally, color variables like `--black` defined as comma-separated RGB values (e.g., `15, 18, 25`) must be wrapped in `rgb()` (e.g., `rgb(var(--black))`) rather than used directly as `var(--black)`, which generates invalid CSS and silently fails to apply the intended color.
**Action:** Always explicitly verify and apply distinct hover styles and transitions to interactive elements like global links and header navigation to improve UX. When modifying default Astro templates, verify the usage of CSS variables for colors to ensure they are properly wrapped in `rgb()` functions if they are defined as comma-separated values.
