...
## 2024-05-18 - Missing visual feedback on link hover states
**Learning:** Default Astro blog templates often lack explicit visual feedback for link hover states (e.g., `a:hover` colors identically match `a`), relying solely on text underlining which can feel disjointed or lack polish. Furthermore, some variables like `--black` require wrapping in `rgb()` to be valid CSS.
**Action:** Always explicitly verify and apply distinct hover styles and transitions (`transition: color 0.2s ease`) to interactive elements like global links and header navigation to improve UX, taking care to ensure CSS variables are correctly formatted.
