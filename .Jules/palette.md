...
## 2024-05-18 - [Interactive Link Hover States]
**Learning:** Default Astro blog templates often lack explicit visual feedback for link hover states (e.g., `a:hover` colors identically match `a`).
**Action:** Always explicitly verify and apply distinct hover styles and transitions to interactive elements like global links and header navigation to improve UX.

## 2024-05-18 - [CSS Variables with RGB Lists]
...
**Action:** Always verify the definition of CSS variables in Astro templates before using them to ensure correct syntax, especially for colors that might require `rgb()` or `rgba()` wrappers.
