...
## 2024-05-19 - Missing hover state feedback on Astro links
**Learning:** Default Astro blog templates often lack explicit visual feedback for link hover states (e.g., `a:hover` colors identically match `a` color). This diminishes perceived interactivity and overall UX.
**Action:** Always explicitly verify and apply distinct hover styles (such as a color shift like `var(--accent-dark)`) and smooth transitions to interactive elements like global links and header navigation to improve UX.
