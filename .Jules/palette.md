...
## 2026-03-02 - Astro Default Link Hover States & CSS Color Variables
**Learning:** Default Astro blog templates often lack explicit hover styles for interactive elements like global links and header navigation, relying on matching standard and hover text colors, leaving minimal visual feedback for users. Furthermore, CSS variables like `--black` defined as comma-separated RGB values (`15, 18, 25`) cause syntax errors when referenced in `.css` or `.astro` styles if they are not explicitly wrapped in the `rgb()` function (e.g. `color: rgb(var(--black));`).
**Action:** Always verify and implement distinct hover styling and state transitions for navigation and text links across components. Additionally, audit the usage of color-based CSS variables in the Astro base template and ensure comma-separated values are utilized strictly within functions like `rgb()` or `rgba()`.
